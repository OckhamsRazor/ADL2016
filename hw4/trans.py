...
def make_sent_batch(sents, embs, n_input, pos, b_size):
    ...
    for b in range(b_size):
        ...
        b_seqlen.append(len(sents[pos+b]))
...
def make_lab_batch(labs, type2id, pos, b_size):
    ...
    for b in range(b_size):
        y = []
        for i, l in enumerate(labs[pos+b]):
            if i == seq_max_len:
                break
            ...
            yy[type2id[l]] = 1
            y.append(yy)
        for _ in range(len(y), seq_max_len):
            y.append([0]*n_classes)
        b_y.append(y)
...
def dynamic_rnn(x, seq_len, W, b, n_hidden, n_input, keep_prob):
    ...
    preds = []
    for o in outputs:
        preds.append(tf.matmul(o, W['out']) + b['out'])

    preds = [tf.nn.dropout(pred, keep_prob) for pred in preds]
    return preds
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-v", "--dev_set", help="validation data",required=True
    )
    parser.add_argument(
        "-c", "--dict_f", help="output dictionary file", required=True
    )
    parser.add_argument(
    ...
    perps = []
    refs = tf.transpose(y, [1,0,2])
    refs = tf.reshape(refs, [-1, n_classes])
    refs = tf.split(0, seq_max_len, refs)
    for logit, target in zip(preds, refs):
        perp = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logit, target))
        perps.append(perp)
    cost = tf.reduce_mean(perps)
