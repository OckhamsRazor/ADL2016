...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "-c", "--class_f", help="training class file", required=True
    )
    parser.add_argument(
        "-o", "--output", help="output", required=True
    )
    parser.add_argument(
        "-m", "--model", help="model dir", required=True
    )
    parser.add_argument(
    ...
    with tf.Session() as sess, open(output, "w") as fout:
        ...
        for pos in range(len(sents)):
            ...
            for i, p in enumerate(pred):
                if i == sent_len:
                    break
                fout.write(id2type[np.argmax(p)])
                if i < sent_len-1:
                    fout.write(" ")
            fout.write("\n")
